...
# Psychedelic mode constants
PSY_FLIP_DURATION = 4 * FPS
PSY_GRAVITY_DURATION = 4 * FPS
PSY_ROTATE_DURATION = 3 * FPS
PSY_COLOR_DURATION = 4 * FPS
PSY_EFFECT_INTERVAL = 240
PSY_MUSIC_INTERVAL = 480
...
class Game:
    def __init__(self):
        ...
        # Psychedelic mode state
        self.psy_screen_flip_h = False
        self.psy_screen_flip_v = False
        self.psy_screen_rotate = 0
        self.psy_gravity_flip = False
        self.psy_color_shift = 0
        self.psy_random_bg_color = None
        self.psy_random_bg_timer = 0
        self.psy_sprite_swap_timer = 0
        self.psy_music_switch_timer = 0
        self.psy_flip_timer = 0
        self.psy_gravity_timer = 0
        self.psy_rotate_timer = 0
        self.psy_color_timer = 0
        self.psy_effect_timer = 0
        self.psy_enabled = False
        self.psy_music_list = []
        ...
        self.initial_background_files = ["y.jpg", "anu.jpg", "anu1.jpg", "anu2.jpg", "anu3.jpg", "anu4.jpg"]
    ...
    def load_assets(self):
        ...
        self.anu_backgrounds = []
        for f in ["anu.jpg", "anu1.jpg", "anu2.jpg", "anu3.jpg", "anu4.jpg"]:
            img = self.load_image(f)
            if img:
        ...
        self.mops_image = self.load_image("mops.jpg")
    ...
    def update_player(
        ...
        if is_crouching:
            ...
            if self.psy_gravity_flip:
                ...
                if y > self.virtual_height - PLAYER_SIZE:
                    y = self.virtual_height - PLAYER_SIZE
                    vel_y = 0
        ...
        player_hitbox_y = (
            y
            + ((PLAYER_SIZE // 2 - player_hitbox_h) if is_crouching else (PLAYER_SIZE - player_hitbox_h) // 2)
        )
    ...
    def handle_bonus_pickup(
        ...
        elif bonus_type == "psy_flip":
            self.psy_screen_flip_v = True
            self.psy_flip_timer = PSY_FLIP_DURATION
            self.bonus_notification = "Переворот экрана!"
        elif bonus_type == "psy_gravity":
            self.psy_gravity_flip = not self.psy_gravity_flip
            self.psy_gravity_timer = PSY_GRAVITY_DURATION
            self.bonus_notification = "Гравитация перевернута!"
        elif bonus_type == "psy_rotate":
            self.psy_screen_rotate = random.choice([90, 180, 270])
            self.psy_rotate_timer = PSY_ROTATE_DURATION
            self.bonus_notification = "Экран повернут!"
        elif bonus_type == "psy_color":
            self.psy_color_shift = random.randint(0, 360)
            self.psy_color_timer = PSY_COLOR_DURATION
            self.bonus_notification = "Цветовой сдвиг!"
    ...
    def apply_psychedelic_effects(self):
        if self.psy_screen_flip_h:
            self.virtual_screen = pygame.transform.flip(
                self.virtual_screen, True, False
            )
        if self.psy_screen_rotate:
            self.virtual_screen = pygame.transform.rotate(
                self.virtual_screen, self.psy_screen_rotate
            )
        if self.psy_color_shift:
            overlay = pygame.Surface(
                (self.virtual_width, self.virtual_height), pygame.SRCALPHA
            )
            r = int(127 + 127 * math.sin(self.psy_color_shift * 0.1))
            g = int(127 + 127 * math.sin(self.psy_color_shift * 0.1 + 2))
            b = int(127 + 127 * math.sin(self.psy_color_shift * 0.1 + 4))
        ...
        for obs in obstacles:
            ...
            if obs_type == "trap":
                ...
                if self.mops_image:
                    try:
                        scaled_texture = pygame.transform.scale(
                            self.mops_image, (visual_w, visual_h)
                        )
                        self.virtual_screen.blit(
                            scaled_texture, (int(visual_x), int(visual_y))
                        )
    ...
    def draw_bonuses(
        ...
        for bonus in bonuses:
            ...
            if bonus_type == "laser":
            ...
            elif bonus_type in ("psy_flip", "psy_gravity", "psy_rotate", "psy_color"):
                colors = {
                    "psy_flip": (255, 0, 255),
                    "psy_gravity": (0, 255, 255),
                    "psy_rotate": (255, 255, 0),
                    "psy_color": (128, 0, 128),
                }
                color = colors.get(bonus_type, (255, 255, 255))
                pygame.draw.circle(
                    self.virtual_screen,
                    color,
                    (
                ...
                # Draw question mark
                q_size = bonus_scaled_size // 2
                q_font = pygame.font.SysFont("arial", q_size, bold=True)
                q_surf = q_font.render("?", True, (255, 255, 255))
                self.virtual_screen.blit(
                    q_surf,
                    (
                        int(bonus_x + bonus_scaled_size // 2 - q_surf.get_width() // 2),
                        int(bonus_y + bonus_scaled_size // 2 - q_surf.get_height() // 2),
                    ),
                )
    ...
    def spawn_obstacle(self):
        ...
        elif random.random() < min(0.3, 0.03 + difficulty_level * 0.01) and self.mops_image:
            w = random.randint(60, 100)
            h = random.randint(60, 100)
            return [
                random.randint(0, self.virtual_width - w),
                -h,
                w,
                h,
                "mops",
    ...
    def spawn_bonus(self):
        if random.random() < LIFE_BONUS_CHANCE:
            ...
            if self.psy_enabled:
                bonus_types += ["psy_flip", "psy_gravity", "psy_rotate", "psy_color"]
    ...
    def spawn_boss_projectile(self):
        if self.coop_mode:
            if not self.player1_is_dead and not self.player2_is_dead:
                target_x = self.player1_x + PLAYER_SIZE // 2
                target_y = self.player1_y + PLAYER_SIZE // 2
            elif not self.player1_is_dead:
                target_x = self.player1_x + PLAYER_SIZE // 2
                target_y = self.player1_y + PLAYER_SIZE // 2
            elif not self.player2_is_dead:
                target_x = self.player2_x + PLAYER_SIZE // 2
                target_y = self.player2_y + PLAYER_SIZE // 2
            else:
    ...
    def update_game(self):
        ...
        if not self.paused and (
            ...
            # Psychedelic mode update
            self.psy_enabled = self.current_mode and self.current_mode["name"] == "Психоделика"
            if self.psy_enabled:
                if not self.psy_music_list:
                    try:
                        self.psy_music_list = sorted([
                            f for f in os.listdir("music")
                            if f.endswith(".mp3") and f not in ("screamer.mp3", "dead.mp3", "bonus.mp3")
                        ])
                    except:
                        self.psy_music_list = []
                self.psy_effect_timer += 1
                if self.psy_effect_timer >= PSY_EFFECT_INTERVAL:
                    self.psy_effect_timer = 0
                    effect = random.choice(["flip", "rotate", "gravity", "color", "music"])
                    if effect == "flip":
                        self.psy_screen_flip_h = random.random() < 0.5
                        self.psy_flip_timer = PSY_FLIP_DURATION
                    elif effect == "rotate":
                        self.psy_screen_rotate = random.choice([90, 270])
                        self.psy_rotate_timer = PSY_ROTATE_DURATION
                    elif effect == "gravity":
                        self.psy_gravity_flip = not self.psy_gravity_flip
                        self.psy_gravity_timer = PSY_GRAVITY_DURATION
                    elif effect == "color":
                        self.psy_color_shift = (self.psy_color_shift + random.randint(60, 180)) % 360
                        self.psy_color_timer = PSY_COLOR_DURATION
                    elif effect == "music":
                        if not self.death_music_played and self.psy_music_list:
                            self.play_music(random.choice(self.psy_music_list))
                # Timers
                for timer_attr in ["psy_flip_timer", "psy_gravity_timer", "psy_rotate_timer", "psy_color_timer"]:
                    val = getattr(self, timer_attr)
                    if val > 0:
                        setattr(self, timer_attr, val - 1)
                    if val <= 0:
                        if timer_attr == "psy_flip_timer":
                            self.psy_screen_flip_h = False
                        elif timer_attr == "psy_gravity_timer":
                            self.psy_gravity_flip = False
                        elif timer_attr == "psy_rotate_timer":
                            self.psy_screen_rotate = 0
                        elif timer_attr == "psy_color_timer":
                            self.psy_color_shift = 0
                # Random music switch every ~6 sec
                self.psy_music_switch_timer += 1
                if self.psy_music_switch_timer >= PSY_MUSIC_INTERVAL:
                    self.psy_music_switch_timer = 0
                    if not self.death_music_played and self.psy_music_list:
                        self.play_music(random.choice(self.psy_music_list))
            else:
...
self.score >= 150
            ...
            if should_spawn:
                if not self.boss_active:
                    ...
                    if random.random() <= self.current_mode["bonus_chance"]:
                        bonus_type = random.choice(["fire", "laser"])
                        y = random.randint(100, self.virtual_height - 100)
                        self.bonuses.append([self.virtual_width, y, bonus_type])
                self.spawn_timer = 0
    ...
    def update_obstacles(self, obstacles, current_obstacle_speed, slowdown_factor):
        for obs in obstacles[:]:
            ...
            if obs_type == "spike":
                ...
                obs[1] += obs[5] * slowdown_factor
                obs[0] -= 0.5 * slowdown_factor
                if obs[1] > self.virtual_height or obs[0] < -obs[2]:
                    obstacles.remove(obs)
                    points = (
                        2 * self.score_multiplier
                        if self.double_points
    ...
    def update_bonuses(self, bonuses, current_obstacle_speed, slowdown_factor):
        for bonus in bonuses[:]:
            ...
            player1_hitbox_y = (
                p1_y
                + (
                    (PLAYER_SIZE // 2 - player1_hitbox_h) if self.player1_is_crouching
                    else (PLAYER_SIZE - player1_hitbox_h) // 2
                )
            )
            ...
            if self.coop_mode:
                ...
                player2_hitbox_y = (
                    p2_y
                    + (
                        (PLAYER_SIZE // 2 - player2_hitbox_h) if self.player2_is_crouching
                        else (PLAYER_SIZE - player2_hitbox_h) // 2
                    )
                )
    ...
    def main(self):
        ...
        while True:
            ...
            if self.state == "welcome":
                ...
                self.draw_game()
                if self.psy_enabled:
                    self.apply_psychedelic_effects()
                if self.paused:
                    self.draw_pause_screen()
            elif self.state == "help":
